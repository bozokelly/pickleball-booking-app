...
export default function ClubCreateForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(successRoute === 'club' ? `/dashboard/club/${club.id}` : '/dashboard/admin');
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
      ...
      {subtitle && <p className="text-sm text-text-secondary -mt-4">{subtitle}</p>}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-2">
            <h3 className="text-sm font-semibold text-text-secondary mb-3">
              Contact Information{contactOptional ? ' (optional)' : ''}
            </h3>
