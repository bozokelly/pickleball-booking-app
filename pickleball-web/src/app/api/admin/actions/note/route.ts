...
// Phase 4A action A2: append an internal note against a flagged row.
// Append-only — the RPC's only write is its own admin_actions row. Available
...
export async function POST(request: NextRequest) {
  ...
  if (!TARGET_TYPES.has(targetType) || !isUuid(targetId) || !note || note.length > 2000) {
  ...
  const { error } = await supabaseAdmin.rpc('admin_add_note', {
  ...
  redirectUrl.searchParams.set('adminAction', 'note_added');
  return NextResponse.redirect(redirectUrl, { status: 303 });
}
